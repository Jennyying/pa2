...
class BBTopo(Topo):
    ...
    def build(self, n=2):
        ...
        self.addLink(hosts[1], switch, bw=args.bw_host, delay=args.delay, max_queue_size=args.maxq)
...
def start_iperf(net):
    ...
    h1 = net.get('h1')
    endHost = h1.popen("iperf -c %s -t %d" % (h2.IP(), args.time), shell=True)
...
def start_ping(net):
    ...
    h1 = net.get('h1')
    h2 = net.get('h2')
    popen = h1.popen("echo '' > %s/ping.txt"%(h2.IP(), args.dir), shell=True)

def curl(net):
    # have a separate function to use the curl command to fetch webpage from the webserver you
    # pawned on host h1 (not from google!)
    h1 = net.get('h1')
    h2 = net.get('h2')
    t = h2.open('curl -o /dev/null -s -w %{time_total} %s/http/index.html' % h1.IP()).communicate()[0]
    return t
...
def bufferbloat():
    ...
    # README and explain.
    fetch_time = np.array(wait_time).astype(np.float)
    print("average of the fetch time: %lf \n" % np.mean(fetch_time))
    print("standard deviation of the fetch time: %lf \n" % np.std(fetch_time))
